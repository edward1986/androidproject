...
class _DetailState extends State<DetailWidget> {
  ...
  FirebaseVisionBarcodeDetector barcodeDetector = FirebaseVisionBarcodeDetector
      .instance;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Column(children: <Widget>[
          buildImage(context),
          widget._scannerType == TEXT_SCANNER ? buildTextList(
              _currentTextLabels) : buildBarcodeList(_currentBarcodeLabels)
        ],));
  }
    ...
    return
      Expanded(
        ...
        child: Container(
            ...
            child: Center(
              child: widget._file == null ? Text('No Image')
                  : FutureBuilder<Size>(
  ...
  Widget buildBarcodeList(List<VisionBarcode> barcodes) {
    if (barcodes.length == 0) {
      return Expanded(
        flex: 1,
        child: Center(
          child: Text('No barcode detected', style: Theme
              .of(context)
              .textTheme
              .subhead),
        ),
  ...
  Widget buildTextList(List<VisionText> texts) {
    if (texts.length == 0) {
      return Expanded(
          flex: 1,
          child: Center(child: Text('No text detected', style: Theme
              .of(context)
              .textTheme
              .subhead),
          ));
...
class _LabelDetectPainter extends BoxPainter {
  ...
  _LabelDetectPainter(texts, originalImageSize)
      : _texts = texts,
        _originalImageSize = originalImageSize;
